...
from sandbox.plasma_usb_reader import PlasmaReading

Phase = Literal["SEEDING", "ASH", "UNFOLDING", "BLOOM"]

@dataclass
class PhoenixState:
    phase: Phase; coherence: float; rebirth_count: int
    last_event: str; fold_verified: bool
...
class PhoenixTrigger:
    ...
    def update(self, plasma: PlasmaReading, contrib: float) -> PhoenixState:
        ...
        if len(self.entropy_window) > 15: self.entropy_window.pop(0)
        ...
        if avg_e >= self.ash_thresh and self.state.phase != "ASH":
            ...
            if self.state.coherence >= self.target:
                self.state.phase = "BLOOM"; self.state.rebirth_count += 1
